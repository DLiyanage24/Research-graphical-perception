...
server <- function(input, output, session) {
  ...
  img_dir <- "/Users/dinuwanthiliyanage/Library/CloudStorage/OneDrive-UniversityofNebraska/Research UNL/Research-graphical-perception/Images"
  ...
  output$rec_timer <- renderText({
    if (!isTRUE(talk_is_recording())) return("")
  ...
  trial_counter_text <- reactive({
    if (rvs$trial_n <= 0) paste0("Trial 0 of ", TOTAL_TRIALS)
    else paste0("Trial ", rvs$trial_n, " of ", TOTAL_TRIALS)
  })
  ...
  observeEvent(input$highlighted_region, {
    ...
    if (is.null(ann) || is.null(ann$polygon)) return()
  ...
  observeEvent(input$show_highlight_demo, {
    showModal(modalDialog(
      ...
      tags$div(style="margin-top:12px; font-size:14px; color:#475569;",
               tags$ul(
